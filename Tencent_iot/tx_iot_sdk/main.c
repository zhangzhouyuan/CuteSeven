...
void on_receive_datapoint(unsigned long long from_id, tx_data_point * data_points, int data_points_count)
{
  ...
  while (i < data_points_count) {
	...
	printf("the msg is from 100, value is:\n\r",from_id);
        printf(data_points[i].value);
        printf("\r\n");
          //on_query_subdevicelist(from_id, data_points[i]);
      }
      ++i;
  }
}
...
bool initDevice() {
    ...
    int nPubkeySize = 0;
    if (!readBufferFromFile("./1700005043.pem", svrPubkey, sizeof(svrPubkey), &nPubkeySize))
    {

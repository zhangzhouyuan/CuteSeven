...
extern void send_audio_alarm_file(char * filepath);
...
extern int cb_on_control_zoom(int zoom);
//===============================================================//

#define FIFO_NAME_1 "/tmp/my_fifo_record_ok"
...
/*    while (scanf("%s", input)) {
    ...
    while(1)
    {
        ...
        if(res>0)
        {
            printf("send voice file[%s]\r\n",buffer);
            system("lame /Robot/voice/wav/listened.wav /tmp/send_tmp.mp3");
            send_audio_alarm_file("/tmp/send_tmp.mp3");
        }

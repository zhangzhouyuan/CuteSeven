...
int msg_id=9999;
char *text="收到";
void ret_callback(unsigned int cookie, int err_code)
{
 printf("send callback cookie[%d],errcode[%d]\r\n",cookie,err_code);
}
unsigned int *cookie;
...
char cmd[1000];
    ...
    fifo_send(cmd);
    tx_send_text_msg(msg_id, text,&ret_callback,cookie,to_targetids,to_targetids_count);
